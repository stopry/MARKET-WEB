$(function () {
    ...
    function caleProfitLoss() {
        var cnt = Number($('#ccsl').html());
        var avgPrice = Number($('#avgPrice').val());
        var price = Number($('#nowPrice').val());
        var fd = (price * cnt - avgPrice * cnt).toFixed(2);
        if (fd < 0) {
            $('#fdykbs').addClass('down');
            $('#fdyksz').addClass('down');
        } else {
            $('#fdykbs').removeClass('down');
            $('#fdyksz').removeClass('down');
        }
        $('#fdyk').html(fd);
    }
    ...
    $('.sellConfirmBtn').on('click', function () {
        ...
        if (para.price > zt) {
            showTips('卖出价格不能高于涨停价');
            return;
        }
        if (para.price < dt) {
            showTips('卖出价格不能低于跌停价');
            return;
        }
        if (para.cnt < minTran) {
